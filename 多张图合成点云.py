...
def reconstruct_from_depth_maps():
    ...
    depth_map_files = sorted(
        [f for f in os.listdir(DEPTH_MAPS_DIR) if f.endswith(".npy")]
    )
    ...
    print(
        f"After color filtering, point cloud has {len(merged_point_cloud.points)} points."
    )
    ...
    print(
        f"After statistical filtering, final point cloud has {len(cleaned_cloud.points)} points."
    )
