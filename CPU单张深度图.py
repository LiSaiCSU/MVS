...
if __name__ == "__main__":
    ...
    for y in tqdm(range(h)):
        for x in range(w):
            ...
            for d in np.linspace(d_start, d_end, num=min(num_steps, 100)): # 限制最大步数以提高效率
                corr = calculate_correlation_score((x, y), d, ref_cam_info, neighbor_cams_info, ref_img, neighbor_images, M_WINDOW_SIZE, NCC_THRESHOLD)
