...
def process_point_cloud(input_file, output_file):
    ...
    # --- 1. 统计离群点移除（第一遍）---
    # 去除初始的、明显的离群噪点
    pcd_cleaned, ind = pcd.remove_statistical_outlier(nb_neighbors=NB_NEIGHBORS_FILTER, std_ratio=STD_RATIO_FILTER)
    print(f"Removed {len(pcd.points) - len(pcd_cleaned.points)} statistical outliers (pass 1).")
    ...
    # --- 3. 统计离群点移除（第二遍）---
    # 再次移除降采样后可能仍然存在的离群点
    pcd_final, ind = pcd_downsampled.remove_statistical_outlier(nb_neighbors=NB_NEIGHBORS_FILTER, std_ratio=STD_RATIO_FILTER)
    print(f"Removed {len(pcd_downsampled.points) - len(pcd_final.points)} statistical outliers (pass 2).")
