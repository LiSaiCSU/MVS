...
# 优化: 深度图后处理滤波
# ----------------------------
def filter_depth_map(depth_map, filter_type='median', median_ksize=5, bilateral_d=5, bilateral_sigma_color=75, bilateral_sigma_space=75):
    """
    对深度图进行后处理滤波以去除噪声。
    参数:
    ...
    if filter_type == 'median':
        if median_ksize % 2 == 0:
            median_ksize += 1
        filtered_map = cv2.medianBlur(filled_depth.astype(np.float32), ksize=median_ksize)
    elif filter_type == 'bilateral':
        # 双边滤波需要深度图为uint8格式，且值范围为0-255，我们先进行归一化
        min_d, max_d = np.percentile(filled_depth[valid_mask], [5, 95])
        normalized_d = (np.clip(filled_depth, min_d, max_d) - min_d) / (max_d - min_d + 1e-6) * 255.0
        normalized_d = normalized_d.astype(np.uint8)
        
        filtered_normalized = cv2.bilateralFilter(normalized_d, d=bilateral_d, sigmaColor=bilateral_sigma_color, sigmaSpace=bilateral_sigma_space)
...
def compute_depth_map_gpu(ref_img_bgr, neighbor_bgr_dict, ref_cam, neighbor_cams,
                          ...
                          DEPTH_CHUNK_SIZE=16, NCC_THRESHOLD=-0.5):
    ...
    idxm1 = idx0 - 1
    idxp1 = idx0 + 1
    depths_triplet = torch.stack([d_samples_all[idxm1], d_samples_all[idx0], d_samples_all[idxp1]], dim=0)
...
if __name__ == "__main__":
    ...
    # --- 1. 计算原始深度图 ---
    raw_depth_map = compute_depth_map_gpu(
        ref_img, neighbor_imgs, ref_cam, neighbors,
        d_min, d_max,
        NUM_DEPTH_COARSE=NUM_DEPTH_COARSE, WIN=WIN,
