...
# --- 新增的并行处理工作函数 ---
def process_row(
    y, w, ref_cam_info, neighbor_cams_info, ref_img, neighbor_images, bbox, params
):
    """
...
if __name__ == "__main__":
    ...
    PARAMS = (10, 0.85, 1e-3)  # M_WINDOW_SIZE  # NCC_THRESHOLD  # DELTA_D_COARSE
    ...
    print(
        f"Generating depth map for {ref_cam_info['name']} ({w}x{h}) using multiprocessing..."
    )
    ...
    if np.any(valid_mask):
        filtered_depth_map[valid_mask] = cv2.medianBlur(
            filtered_depth_map[valid_mask].astype(np.float32), 5
        )
    # --- 后处理结束 ---
    # --- 5. 保存和显示结果 (与之前相同) ---
